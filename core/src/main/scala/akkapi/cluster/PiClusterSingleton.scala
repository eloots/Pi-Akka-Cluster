...
object PiClusterSingleton {
  ...
  def apply(clusterStatusTracker: ActorRef[ClusterStatusTracker.ClusterEvent]): Behavior[Command] = {
    Behaviors.setup { context =>
      new PiClusterSingleton(context, clusterStatusTracker).run()
    }
  }
}

class PiClusterSingleton private (context: ActorContext[PiClusterSingleton.Command], clusterStatusTracker: ActorRef[ClusterStatusTracker.ClusterEvent]) {
