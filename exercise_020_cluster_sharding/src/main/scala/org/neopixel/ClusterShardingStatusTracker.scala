...
object ClusterShardingStatusTracker {
  ...
  def props(): Props = Props(new ClusterShardingStatusTracker)
}

class ClusterShardingStatusTracker extends Actor with ActorLogging with SettingsActor with Timers {
  import ClusterShardingStatusTracker._
  ...
  def running(heartbeatLEDOn: Boolean, weaklyUpIndicatorOn: Boolean, weaklyUpMembers: Set[Member]): Receive = {
    ...
    case ShardingCoordinatorTracker.ShardingCoordinatorRunning =>
      setPixelColorAndShow(strip, 1, LightBlue)

    case ShardingCoordinatorTracker.NoShardingCoordinatorRunning =>
  ...
  override def preStart(): Unit = {
    ...
    context.actorOf(ShardingCoordinatorTracker.props, "sharding-coordinator-tracker")
